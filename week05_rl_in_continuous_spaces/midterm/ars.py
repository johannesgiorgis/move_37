...
class ArsTrainer():
    ...
    def train(self):
        ...
        t = trange(self.hp.nb_steps, desc=desc, leave=True)
        r_len = len(str(self.hp.nb_steps))
        for step in t:
            ...
            #print(
            #    f"Step: {step:>{report_width}}/{self.hp.nb_steps:>{report_width}} "
            #    f"Reward: {reward_evaluation:3.4f}")
            self.record_video = False
