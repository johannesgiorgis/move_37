...
def calc_harmonic_ring_score(rings):
    multiplier = 10
    return round(float(sum(Fraction(1, d) for d in range(1, rings + 1))) * multiplier)
...
def eval_genomes(genomes, config):
    ...
    for genome_id, genome in genomes:
        ...
        while not done:
            ...
            if rings > rings_max:
                fitness_current += calc_harmonic_ring_score(rings)
                rings_max = rings
